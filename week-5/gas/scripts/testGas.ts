// Use ethers in hardhat.config.ts to compare gas usage in different scenarios
...
//     console.log(`Using ${userSettings.settings?.optimizer.runs} runs optimization`);
//   }
...
//   const testTx1Receipt = await testTx1.wait();
//   console.log(`Used ${testTx1Receipt?.gasUsed} gas units in struct packing test function`);
//   const testTx2 = await contract.createPacked();
//   const testTx2Receipt = await testTx2.wait();
//   console.log(`Used ${testTx2Receipt?.gasUsed} gas units in optimized struct packing test function`);
// }

// compareDeploy().then(() => console.log("-----"));
// compareLocations().then(() => console.log("-----"));
// comparePacking().then(() => console.log("-----"));
