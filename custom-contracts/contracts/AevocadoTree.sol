...
contract AevocadoTree is ERC20, ERC20Permit {
    ...
    address public constant AEVO_CONTRACT_ADDRESS = 0xB528edBef013aff855ac3c50b381f253aF13b997;
    uint256 public constant EXCHANGE_RATIO = 100; // 100 AEVO = 1 AEVOCADO
    uint256 public constant MAX_SUPPLY = 1000000 * 10**18; // 1 million AEVOCADO

    event AevoDeposited(address indexed user, uint256 aevoAmount, uint256 aevocadoAmount);
    event AevocadoConverted(address indexed user, uint256 aevocadoAmount, uint256 aevoAmount);

    constructor() ERC20("AevocadoTree", "AEVOCADO") ERC20Permit("AevocadoTree") {}

    function depositAevoForAevocado(uint256 aevoAmount) external {
        require(aevoAmount > 0, "Amount must be greater than 0");
        require(aevoAmount % EXCHANGE_RATIO == 0, "Amount must be divisible by 100");

        uint256 aevocadoAmount = aevoAmount / EXCHANGE_RATIO;
        require(totalSupply() + aevocadoAmount <= MAX_SUPPLY, "Exceeds max supply");
    ...
    function convertAevocadoToAevo(uint256 aevocadoAmount) external {
        ...
        IERC20 aevoToken = IERC20(AEVO_CONTRACT_ADDRESS);
        require(aevoToken.balanceOf(address(this)) >= aevoAmount, "Insufficient AEVO balance in contract");
