...
describe("HelloWorld", function () {
  ...
  it("Should not allow anyone other than owner to call transferOwnership", async function () {
    ...
    const helloWorldContractAsOtherAccount = await viem.getContractAt("HelloWorld", helloWorldContract.address, {
      client: { wallet: otherAccount },
    });
    // https://www.chaijs.com/plugins/chai-as-promised/
    await expect(
      helloWorldContractAsOtherAccount.write.transferOwnership([otherAccount.account.address])
    ).to.be.rejectedWith("Caller is not the owner");
  });

  it("Should execute transferOwnership correctly", async function () {
    const { publicClient, helloWorldContract, owner, otherAccount } = await loadFixture(deployContractFixture);
    // https://hardhat.org/hardhat-runner/plugins/nomicfoundation-hardhat-viem#contracts
    const txHash = await helloWorldContract.write.transferOwnership([otherAccount.account.address]);
    ...
    const helloWorldContractAsPreviousAccount = await viem.getContractAt("HelloWorld", helloWorldContract.address, {
      client: { wallet: owner },
    });
    await expect(
      helloWorldContractAsPreviousAccount.write.transferOwnership([owner.account.address])
    ).to.be.rejectedWith("Caller is not the owner");
  });
  ...
  it("Should not allow anyone other than owner to change text", async function () {
    ...
    // Set new text in contract as owner
    const helloWorldContractAsOwner = await viem.getContractAt("HelloWorld", helloWorldContract.address, {
      client: { wallet: owner },
    });

    await helloWorldContractAsOwner.write.setText(["New Text As Owner"]);
    const helloWorldText = await helloWorldContract.read.helloWorld();
    expect(helloWorldText).to.equal("New Text As Owner");
    ...
    // Try set new text in contract again as old owner
    await expect(helloWorldContractAsOwner.write.setText(["New Text as Old Owner"])).to.be.rejectedWith(
